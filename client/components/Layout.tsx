...
export default function Layout({ children }: LayoutProps) {
  ...
  const navLinks = [
    ...
    { href: "/login", label: "Login", public: true, hideWhenLoggedIn: true },
    { href: "/register", label: "Register", public: true, hideWhenLoggedIn: true },
  ];

  const dashboardLinks = isLoggedIn
    ? [
        userRole === "admin" && { href: "/admin", label: "Admin Dashboard" },
        userRole === "institution" && {
          href: "/institution",
          label: "Institution Dashboard",
        },
        userRole === "donator" && { href: "/donator", label: "Donator Dashboard" },
      ].filter(Boolean)
    : [];
      ...
      <footer className="border-t border-border bg-card">
        <div className="khair-container py-12">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-lg font-bold">KhairConnect</span>
              </div>
              <p className="text-sm text-muted-foreground">
                Coordinating donations for charitable impact through weekly rotation
          ...
          <div className="border-t border-border mt-8 pt-8 flex flex-col sm:flex-row items-center justify-between text-sm text-muted-foreground">
            <p>&copy; {new Date().getFullYear()} KhairConnect. All rights reserved.</p>
            <p>Building communities through charitable coordination.</p>
          </div>
        </div>
      </footer>
    </div>
