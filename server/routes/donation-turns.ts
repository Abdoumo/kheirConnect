...
export const getDonationHistory: RequestHandler = async (
  req: AuthRequest,
  res
) => {
  try {
    ...
    // Get all donation turns (completed and accepted) for this donator
    const turns = await DonationTurn.find({
      donatorId: userId,
    ...
    return res.json(
      turns.map((turn) => ({
        _id: turn._id,
        institutionId: (turn.institutionId as any)?._id,
        institutionName: (turn.institutionId as any)?.name,
        institutionLocation: (turn.institutionId as any)?.location,
        week: turn.week,
        weeklyNeeds: turn.weeklyNeeds,
